...
- **NEXT_PUBLIC_GA_MEASUREMENT_ID:** Google Analytics 4 Measurement ID (e.g. `G-XXXXXXXXXX`). If set, gtag.js is loaded for GA4. After a successful lead email via EmailJS, the app sends **exactly one** event per lead: **`vyplneny_formular`** for the homepage calculator (full form; optional `value`/`currency` in CZK), or **`telefonni_cislo`** for phone-only flows (popup + CTA on Kontakty / Jak to funguje; `lead_source` = `popup` or `cta`, optional `page_path`). GA4 event names use underscores only; these are the equivalents of “vyplněný formulář” vs “telefonní číslo”. Mark each as a key event in GA4 if you want separate conversions. In Google Ads, **import** the GA4 event(s) you need—do not also set **NEXT_PUBLIC_GOOGLE_ADS_LEAD_CONVERSION_SEND_TO** for the same goal, or you risk double counting.
- **NEXT_PUBLIC_GOOGLE_ADS_LEAD_CONVERSION_SEND_TO:** Optional. Full Google Ads conversion `send_to` string (`AW-XXXXXXXXX/label`). If set, the app fires a **`conversion`** event after each successful lead. Use only if you are **not** importing the matching GA4 event(s) into Google Ads for the same lead action.
- **NEXT_PUBLIC_EMAILJS_***:** Required for contact forms. Public key, service ID, and template ID from [emailjs.com](https://www.emailjs.com/). Template variables: `{{source}}` (in the app this is the public site URL, e.g. `Odesláno z: https://hnedpenize.cz` — **include `{{source}}` in the EmailJS HTML body** so it appears in the inbox), `{{phone}}`, `{{name}}`, `{{email}}`, `{{amount}}`, `{{assetType}}` / `{{collateralType}}` (Typ zajištění: Nemovitost / Automobil), `{{serviceType}}`. Reference HTML for the lead email lives in **`emailjs/lead-notification-template.html`** (paste into the dashboard or merge `{{source}}` into your existing template). For CTA/popup (phone-only “callback”) the app sends: name/assetType `---`, serviceType `Není relevantní (Callback)`, amount `--- Pouze požadavek na zavolání ---`; use these in one template to get the “NOVÁ POPTÁVKA K POSOUZENÍ” callback email format.
...
1. Set **`NEXT_PUBLIC_GA_MEASUREMENT_ID`**, rebuild, and deploy. Submit test leads; in GA4 use **Realtime** or **DebugView** to confirm **`vyplneny_formular`** (calculator) and **`telefonni_cislo`** (popup/CTA).
2. In GA4 **Admin**, open **Events** (or **Key events**), find **`vyplneny_formular`** and **`telefonni_cislo`**, and mark each as a **key event** if you want separate conversions.
3. In Google Ads, under **Linked accounts**, link the **same** GA4 property. Under **Conversions**, create an **Import** from **Google Analytics 4** and import **`vyplneny_formular`** and/or **`telefonni_cislo`** as separate conversion actions.
